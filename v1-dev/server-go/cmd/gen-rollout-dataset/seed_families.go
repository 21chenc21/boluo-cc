...
// ============ F11: 死牌×对子行位 (129/130 家族, v1-dev 2026-07-11) ============
// 病理 (2026-07-10 验尸): V组特征(p对升trips)干净饱满 — 130病线 p中升trips=0.000 特征在尖叫 —
// 但 value head 响应倒挂: 对子出路死得越多越该封顶对下底/出中, NN反而留中垫烂底 (删死牌d5自摆对).
// 家族 = 响应函数重校准: 对子rank(5..Q) × 出路死牌0~2 × 余牌结构(压秤/带大牌) 全随机, label=rollout真值.
// 扰动增强内建: extraUsed 再塞 0~2 张无关死牌 — "无关死牌不改摆法"直接写进数据分布 (边界摆动测试=验收).
func seedPairPlace(rng *rand.Rand) *seedSpec {
	p := newCardPool(rng)
	s := &seedSpec{family: "pairPlace", startRound: 1}
	pr := 3 + rng.Intn(8) // 对子 rank 5..Q
	s.dealt = p.takeN(pr, 2)
	switch rng.Intn(3) {
	case 0:
		// 129/130型: 对子压秤 (其余全 < pr)
		for i := 0; i < 3; i++ {
			s.dealt = append(s.dealt, p.takeLow(0, pr-1))
		}
	case 1:
		// 带1张大牌 (顶/底发育材料)
		s.dealt = append(s.dealt, p.take(pickRank(rng, pr+1, 12)))
		s.dealt = append(s.dealt, p.takeLow(0, pr-1), p.takeLow(0, pr-1))
	default:
		// 131型: 小对 + 双大牌 (K/Q/A 级, 该都沉底) + 1杂. pr 收窄到小对 3..8.
		if pr > 6 {
			pr = 3 + rng.Intn(4) // 5..8
			s.dealt = p.takeN(pr, 2)
		}
		s.dealt = append(s.dealt, p.take(pickRank(rng, 10, 12)), p.take(pickRank(rng, 10, 12)))
		s.dealt = append(s.dealt, p.takeLow(0, pr-1))
	}
	// 核心轴: 对子升级出路死牌 0~2 张
	for i := rng.Intn(3); i > 0; i-- {
		s.extraUsed = append(s.extraUsed, p.take(pr))
	}
	// 扰动增强: 0~2 张无关死牌
	for i := rng.Intn(3); i > 0; i-- {
		s.extraUsed = append(s.extraUsed, p.takeLow(0, 12, pr))
	}
	return s
}
