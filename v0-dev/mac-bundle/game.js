...
function checkFantasyTrigger(playerState, fromFantasy) {
    ...
    if (topEval.type === 1) {
        // 提取顶对 rank (joker-aware)
        const realCnts = {};
        const jokerCnt = top.filter(c => c.rank === 'X').length;
        for (const c of top) if (c.rank !== 'X') realCnts[c.rank] = (realCnts[c.rank] || 0) + 1;
        let pairR = -1;
        for (const [r, v] of Object.entries(realCnts)) {
            if (v + jokerCnt >= 2) pairR = Math.max(pairR, rankIndex(r));
        }
        if (jokerCnt >= 2 && pairR < 0) pairR = 12;
