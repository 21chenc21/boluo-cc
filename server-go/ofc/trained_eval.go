...
func BuildFeatures(gs *GameState, inDim int) []float32 {
	...
	if inDim == 168 {
		// 旧 sp37-sp38 layout (无 FE 组范EV dim168). 新 169-d 的前 168 维 layout 不变 → 截前 168.
		// 兼容 sp38 太子 (gen rollout / bench). ⚠️ O组147-149 已清零(sp39), sp38 ckpt 看到零值属预期 OOD.
		return BuildFeaturesV3(gs)[:168]
	}
	if inDim == 165 {
	...
	if inDim == 161 {
		// 旧 sp28 layout (无 B 组顺紧密度). 新 164-d 的前 161 维不变 → 截前 161. 兼容旧 161-d ckpt(iter-*).
		return BuildFeaturesV3(gs)[:161]
	}
	if inDim == 150 {
		// 旧 sp26 layout (无 W 组前瞻支撑夹缝). 新 154-d 的前 150 维不变 → 截前 150. 兼容旧 150-d ckpt.
		return BuildFeaturesV3(gs)[:150]
	}
