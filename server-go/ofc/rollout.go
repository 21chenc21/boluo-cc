...
func (er *ExpertRollout) QuickRollout(state *GameState, currentRound int) float32 {
	...
	if gs.IsComplete() {
		...
		// 2026-06-25 用户: 成手奖励 (silver-label only) — 低端成手(中对/两对, 底两对/三条)补 equity 分,
		//   治 value-head "不珍惜成手"(#66 中两对没 royalty 被散成垃圾顶). 不是真实对局分.
		mhr := MadeHandRewardLabel(score.TopEval, score.MidEval, score.BotEval)
