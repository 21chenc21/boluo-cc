...
func main() {
	...
	// 2026-06-20 (用户): duel 也读规则开关 — 纯NN 训练 duel 必须纯NN (硬+软+支配全关), 否则 fantasy/score 比的是 NN+规则.
	if os.Getenv("DISABLE_HARD_RULES") != "" {
		ofc.HardRulesDisabled = true
		log.Println("[duel] DISABLE_HARD_RULES=1: 关候选filter+支配过滤")
	}
	if os.Getenv("DISABLE_SOFT_RULES") != "" {
		ofc.SoftRulesDisabled = true
		log.Println("[duel] DISABLE_SOFT_RULES=1: 关全部软规则")
	}
