...
func main() {
	...
	// 2026-06-20 (用户): BUG FIX — 之前只设 HardRulesDisabled, 日志却说"关软硬规则" = 撒谎.
	//   软规则(含 rnSoftScore bonus/penalty)其实还在影响 rollout 策略 → silver-label EV 带软规则偏置 (i154 白训根因).
	//   现独立读 DISABLE_SOFT_RULES. 支配过滤在 HardRulesDisabled 下, 已被 DISABLE_HARD_RULES 关.
	if os.Getenv("DISABLE_HARD_RULES") != "" {
		ofc.HardRulesDisabled = true
		log.Println("[gen] DISABLE_HARD_RULES=1: 关候选 filter + 支配过滤")
	}
	if os.Getenv("DISABLE_SOFT_RULES") != "" {
		ofc.SoftRulesDisabled = true
		log.Println("[gen] DISABLE_SOFT_RULES=1: 关全部软 bonus/penalty (纯 value-head rollout)")
	}
	...
	progressEvery := *numGames / 20 // 总共约 20 行进度, 防 500 局刷屏
	if progressEvery < 1 {
		progressEvery = 1
	}
