...
func main() {
	...
	for i := range actions {
		...
		c.score = c.te + ofc.PolicyBoost*c.plogit - c.foul - c.kkOnMid + c.singleA - ofc.RnJokersSameRowPenalty(a, gs) + ofc.RnSingleJokerTopChaseABonus(gs, state) - ofc.RnLoneAceMidJokerTopPenalty(gs, state) + ofc.RnTopTripsFantasyBonus(gs) - ofc.RnTopTripsOvercommitPenalty(gs, state) + ofc.RnJokerAOnTopBonus(a, gs) - ofc.RnQuadsJokerWastePenalty(gs) - ofc.RnMidExceedsBotPenalty(gs) + ofc.RnBotMakeTwoPairBonus(gs, state) + ofc.RnMidMakeTwoPairBonus(gs, state) + ofc.RnPreserveTopAAChaseBonus(gs) - ofc.RnMidHighCardOverBotPenalty(gs, state) - ofc.RnLoneSubQOnTopPenalty(gs, state)
		c.hardRuleOK = true
