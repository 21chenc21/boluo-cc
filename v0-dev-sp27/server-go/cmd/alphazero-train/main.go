...
var (
	...
	foulCost      = flag.Float64("foul-cost", 3, "") // sp28: foul -3
	fanBonusQQ    = flag.Float64("fan-bonus-qq", 10, "")
	fanBonusKK    = flag.Float64("fan-bonus-kk", 30, "")
	...
	casesFile     = flag.String("cases-file", "", "testcase JSON path (relative to cwd). \"\" disables testcase bench. sp28: 默认空(删门禁)")
...
func main() {
	...
	// sp28: 纯NN 训练 — gen/duel 关掉所有规则 (env DISABLE_HARD_RULES/DISABLE_SOFT_RULES=1).
	// 铁律: 带规则 gen 会污染标签 (规则救场洗掉 -foul 信号 + 裁剪删坏候选无 label). 默认纯NN.
	if os.Getenv("DISABLE_HARD_RULES") != "0" {
		ofc.HardRulesDisabled = true
	}
	if os.Getenv("DISABLE_SOFT_RULES") != "0" {
		ofc.SoftRulesDisabled = true
	}
	log.Printf("[az] sp28 纯NN gen: HardRulesDisabled=%v SoftRulesDisabled=%v", ofc.HardRulesDisabled, ofc.SoftRulesDisabled)
	...
	for iter := 0; iter < *iters; iter++ {
		...
		log.Printf("[az iter %d] duel: new AA+三条=%d fan=%d score=%.1f foul=%d | best AA+三条=%d fan=%d score=%.1f foul=%d (games=%d)",
			iter+1,
			ds.aaTripsA, ds.fanA, ds.scoreA, ds.foulA,
			ds.aaTripsB, ds.fanB, ds.scoreB, ds.foulB, ds.games)

		// sp28: 唯一 best 判定 = AA对+三条 进范数 ↑ → PROMOTE.
		//   删掉 testcase 门禁 / 总 fantasy 数 / score tiebreak. 只认 AA+三条进范多.
		promote := ds.aaTripsA > ds.aaTripsB
		reason := fmt.Sprintf("AA+三条进范 %d→%d", ds.aaTripsB, ds.aaTripsA)
		...
		if promote {
			...
			log.Printf("[az iter %d] ✗ DISCARD new (AA+三条进范 未严格 > best)", iter+1)
		}
	}
...
type duelResult struct {
	score   float32
	fantasy bool
	aaTrips bool // 顶范是 AA对 或 三条 (sp28 best 判定专用, 不含 QQ/KK)
	foul    bool
}
...
type duelStats struct {
	games              int
	fanA, fanB         int     // 进范局数
	aaTripsA, aaTripsB int     // AA+三条 进范局数 (sp28 唯一 best 判定指标)
...
func duelTwo(ckptA, ckptB string, numGames, mctsSims int, cfg *ofc.RolloutConfig, jokers, phantomOpp, workers int, rng *rand.Rand) duelStats {
	...
	for i := 0; i < numGames; i++ {
		...
		if resultsA[i].aaTrips {
			stats.aaTripsA++
		}
		if resultsB[i].aaTrips {
			stats.aaTripsB++
		}
		if resultsA[i].foul {
			stats.foulA++
...
func duelPlayGame(deck []ofc.Card, opp, slot int, cfg *ofc.RolloutConfig, mctsSims, jokers int, rng *rand.Rand) duelResult {
	...
	if score.Fantasy {
		...
		raw += fb
		// sp28: 用 tier sentinel (QQ=1 KK=2 AA=3 trips=4) 判顶范是否 AA对/三条
		tier, _ := ofc.FantasyBonusFromBoard(state.Top, state.Middle, state.Bottom, 1, 2, 3, 4)
		return duelResult{score: raw, fantasy: true, aaTrips: tier >= 3}
	}
