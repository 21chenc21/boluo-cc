...
func jokerEffRank(cards []Card, ev HandValue) float32 {
	...
	// makeValue 编码: v = type*1e6 + topRank*15^4 + 次kicker*15^3 + ...
	// 顶组 rank(对子/三条/金刚的对子rank, 顺花的最高张, 高牌的最高kicker) 在 15^4=50625 位.
	// 2026-06-21 BUG fix: 旧版用 /15 (取最低kicker) → 鬼成对/三条时返回 ~39510 垃圾值, f[37-39] 全坏.
	topRank := int((ev.Value % 1000000) / 50625)
	if topRank < 0 {
		topRank = 0
	}
	if topRank > int(RankA) {
		topRank = int(RankA)
	}
	return float32(topRank)
}
