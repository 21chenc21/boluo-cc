...
//   - 默认 cases/game-cases.json (183 case 合并集)
//
// 输出:
...
//
// 用法:
//   node test-cases.js                                # 默认 game-cases.json
//   node test-cases.js cases/hard.json
//   node test-cases.js cases/all-tests.json cases/hard.json
//   node test-cases.js --url=http://localhost:18001 --runs=5 cases/game-cases.json
...
for (const arg of process.argv.slice(2)) {
    if (arg.startsWith('--url=')) URL_STR = arg.slice(6);
        ...
        console.log('  默认: cases/game-cases.json, runs=1, url=http://localhost:8002');
        process.exit(0);
    } else files.push(arg);
}
if (files.length === 0) {
