...
#
# 默认 cases: cases/game-cases.json
# 默认 runs: 1
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        -h|--help)
            ...
            echo "  默认 cases: cases/game-cases.json"
...
if [ ${#CKPTS[@]} -eq 1 ]; then
    ...
    REPORT="$CKPT_DIR/${CKPT_NAME}-test-${TS}.md"
    {
        ...
        echo
        echo "- ckpt: \`$CKPT\`"
        echo "- runs: $RUNS, cases: ${CASES[*]:-cases/game-cases.json (183 合并集)}"
        echo "- timestamp: $TS"
        echo
        echo '```'
    } > "$REPORT"
    echo "[run-cases] ckpt: $CKPT_NAME, port: $PORT, runs: $RUNS"
    echo "[run-cases] cases: ${CASES[*]:-cases/game-cases.json (183 合并集)}"
...

{
    ...
    echo
    echo "- runs/ckpt: $RUNS"
    echo "- cases: ${CASES[*]:-cases/game-cases.json (183 合并集)}"
    echo
...
PY
    ...
    local RAW="$OD/raw.txt"
    {
        ...
        echo "- runs: $RUNS, cases: ${CASES[*]:-cases/game-cases.json}"
        echo
        echo '```'
    } > "$DETAIL"
